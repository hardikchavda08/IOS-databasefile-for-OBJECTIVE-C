...
NSString * const DataBaseName  = @"Student.db"; // Paas Your DataBase Name Over here
...
-(BOOL)createNewTable:(NSString*)querys
{
    ...
    if (sqlite3_open([path UTF8String], &databaseObj) == SQLITE_OK) {
        if (createStmt == nil) {
            ...
            if (sqlite3_prepare_v2(databaseObj, [querys UTF8String], -1, &createStmt, NULL) != SQLITE_OK) {
                return NO;
            }
            sqlite3_exec(databaseObj, [querys UTF8String], NULL, NULL, NULL);
            return YES;
        }
    }
    return YES;
